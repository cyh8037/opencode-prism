...
export function createVisionLookTool(pipeline: VisionPipeline): ToolDefinition {
  return tool({
    description:
      '用视觉模型解读图片，返回与 goal 相关的描述。images 支持 URL、data URL、本地文件路径（相对路径按项目目录解析），以及哨兵值 "last"（单独使用时表示"本会话最近的一张图片"——用户在对话里贴图/截图而你无法直接查看图片内容时，用它解读）。goal 建议始终提供：说明你要从图片里得到什么（如"找出报错信息""对比与设计稿的布局差异"），解读会只回答关注点相关内容。',
    args: {
      images: tool.schema.array(tool.schema.string()).describe('图片 URL、data URL、本地路径列表；或 ["last"] 表示会话内最近的图片'),
    ...
    async execute(args: { images: string[]; goal?: string }, ctx) {
      try {
        if (args.images.length === 1 && args.images[0] === "last") {
          ...
          return result.text ?? VISION_NO_MODEL_HINT
        }
        const images = args.images.map((url) => ({ mime: guessImageMime(url), url }))
        const text = await pipeline.look(ctx.sessionID, images, args.goal)
        return text ?? VISION_NO_MODEL_HINT
      } catch (error) {
